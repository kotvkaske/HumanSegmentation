...
def generate_html(path_to_data):
    ...
    paths_to_imgs = sorted(
        ["/".join(path.split("/")[-1:]) for path in glob(f"{path_to_data}/*_img.jpg")])

    paths_to_masks = sorted(
        ["/".join(path.split("/")[-1:]) for path in glob(f"{path_to_data}/*_pred_mask.png")])
    paths_to_crops = sorted(
        ["/".join(path.split("/")[-1:]) for path in glob(f"{path_to_data}/*_crop.png")])
